const {GatewayIntents, Endpoints, TimeoutError, Client, LoggerFactory, LoggerLevel} = require('../disjs/index');
...
(async () => {
    ...
    const client = new Client({
        ...
        intents: GatewayIntents.MESSAGE_CONTENT | GatewayIntents.GUILD_MESSAGES | GatewayIntents.GUILDS,
        shard: true,
        loggerFactory: new LoggerFactory({level: LoggerLevel.TRACE}),
    });
    if (argv[2] === 'sync') {
        console.log('Syncing...');
    ...
    // console.log(await client.rest.request(Endpoints.GET_CURRENT_USER()).then(r => r.json()));
        // shard.listen('GUILD_CREATE',)
    client.listen('READY', ({data: {user}}) => {
        console.log(`Logged as ${user.username}#${user.discriminator}!`);
    });
    client.listen('INTERACTION_CREATE', async ({data: {id, type, data, member, token}}) => {
        switch (type) {
        case 2: {
    ...
    client.listen('MESSAGE_CREATE', async (data) => {
